...
def _source_dar(v: dict, w: int, h: int) -> tuple[int, int]:
    """Source display aspect ratio as (num, den), reduced."""
    from math import gcd
    sar_str = v.get("sample_aspect_ratio") or ""
    if sar_str and ":" in sar_str:
        try:
            sn, sd = (int(x) for x in sar_str.split(":"))
            if sn > 0 and sd > 0 and w > 0 and h > 0:
                num, den = w * sn, h * sd
                g = gcd(num, den) or 1
                return num // g, den // g
        except (ValueError, ZeroDivisionError):
            pass
    dar_str = v.get("display_aspect_ratio") or ""
    if dar_str and ":" in dar_str:
        try:
            dn, dd = (int(x) for x in dar_str.split(":"))
            if dn > 0 and dd > 0:
                return dn, dd
        except (ValueError, ZeroDivisionError):
            pass
...
def probe_summary(info: dict) -> dict:
    ...
    dar_n, dar_d = _source_dar(v, w, h)
    return {
        "codec_v": v.get("codec_name"),
...
async def probe_and_emit(job: Job) -> None:
    info = probe_summary(await probe(job.src))
    job.info = info
    await state.broadcast({"type": "info", "id": job.id, "info": info})
...
def _run_native_window_gtk(url: str) -> int:
    """Open the web UI inside a GTK4 + WebKit window (Linux). No system browser."""
    import gi
    gi.require_version("Gtk", "4.0")
    gi.require_version("WebKit", "6.0")
    from gi.repository import Gtk, WebKit, Gio  # type: ignore

    app_id = "com.audi.MmiMib1Converter"
    gtk_app = Gtk.Application.new(app_id, Gio.ApplicationFlags.NON_UNIQUE)

    def on_activate(application):
        win = Gtk.ApplicationWindow(application=application)
        win.set_title("Audi MMI MIB1 Converter")
...
def _run_native_window(url: str) -> int:
    if sys.platform == "win32":
        return _run_native_window_pywebview(url)
    return _run_native_window_gtk(url)
...
def main() -> int:
    ...
    no_window = "--no-window" in sys.argv or os.environ.get("AUDI_NO_WINDOW") == "1"
